...
for n in range(10, X_train.shape[1], 100):
    # 按照一定比例选取特征
    X_train_selected = SelectKBest(mutual_info_regression, k=n).fit_transform(X_train, y_train)
    # 得到列数，即特征数目
...
plt.xlabel("The Number of Descriptors(Selected by Mutual Information)")
...
NumberOfDescriptors.to_csv('./PHD2_data/feature_selection_mutual_EGLN1_std.csv',index=False)